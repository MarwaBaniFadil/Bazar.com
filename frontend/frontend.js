...
const port= 5002;                           // the port for front end server is 3000

app.get('/search/:topic',(req,res)=>{       // if the req is search req route it based on topic
    try {
    http.get('http://catalog:5000/search/'+req.params.topic,( response)=>{         // get req to the catalog server to return the item
...
app.get('/info/:item_number',(req,res)=>{                                           // if the req is info req route it based on item number          
    try {
    http.get('http://catalog:5000/info/'+req.params.item_number,(response)=>{     // get req to the catalog server to return the information about the item
        response.on("data", (chunk)=>{
...
app.post('/purchase/:item_number', async (req,res)=>{                                                       // if the req is purchase req route it based on item number     
    try {
        const response = await axios.post(`http://order:5001/purchase/${req.params.item_number}`);      // make an http post req to order server using axios
